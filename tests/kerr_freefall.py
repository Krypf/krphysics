...
def kerr_inv(r, th):
    ...
    Dlt  = r**2 + a**2 - 2*m*r
    A_   = (r**2+a**2)**2 - a**2*Dlt*np.sin(th)**2
    s2   = np.sin(th)**2
    gtt   = -A_/(Sig*Dlt)
    gtphi = -2*a*m*r/(Sig*Dlt)
    grr   =  Dlt/Sig
    gthth =  1.0/Sig
    gphph = (Dlt - a**2*s2)/(Sig*Dlt*s2) if abs(s2) > 1e-10 else 1e10
    return gtt, gtphi, grr, gthth, gphph, Sig, Dlt
...
def make_rhs(use_plasma):
    def rhs(s, y):
        ...
        if r < r_h * 1.005:
            return [0.0]*6
        gtt, gtph, grr, gthth, gphph, Sig, Dlt = kerr_inv(r, th)
...
# impact parameters
# ============================================================
pph_values = [2.5, 3.0, 3.5, 4.0, 5.0, 6.0, 8.0, 12.0, 20.0]
cmap   = plt.cm.plasma
...
fig.suptitle(
    r'Ray Tracing in Kerr Spacetime ($a=0.9M$)  —  '
    r'Free-fall plasma: $n(r,\theta)\propto[r(r^2+a^2)]^{-1/2}$',
    fontsize=12, y=1.01, color='black')

plt.savefig('tests/kerr_freefall_raytracing.png',
