...
function main()
    ...
    # salect the folder path
    folder = "julia/lattice/"
    savefig(plot, folder * "fibonacci_lattice.png")  # Saves the plot as a PNG file
    
    lattice_disk = square_to_disk.(lattice)
    plot = scatter(lattice_disk, label="Grid Points", aspect_ratio=:equal, color=colors[1:n])
    savefig(plot, folder * "fibonacci_disk.png")  # Saves the plot as a PNG file
    ...
    savefig(plot, folder * "fibonacci_sphere.png")  # Saves the plot as a PNG file
